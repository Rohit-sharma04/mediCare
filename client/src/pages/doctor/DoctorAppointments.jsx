...
const DoctorAppointments = () => {
  ...
  const handleAccountStatus = async (appointment, status) => {
    try {
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
           {appointments.length==0 ?
            <td className=" ol-start-2 col-span-4">
            <img src={noData} className="w-full " alt="no Data"/> </td> :
              ...
              return (
                <tr key={appointment._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td className="px-6 py-4">
                    <span className={(appointment.status) === 'pending' ? 'text-yellow-500' : (appointment.status) === 'rejected'? 'text-red-600':'text-green-500'}>
                      {appointment.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {appointment.status === "approved" ? (
                      ...
                      onClick={() => handleAccountStatus(appointment, "rejected")}
                      >Reject</button>
                     
                    ) : (//else status=pending"||"rejected"
                      <button
                        className="btn btn-success "
                        onClick={() => handleAccountStatus(appointment, "approved")}
                      >
                        Approve
                      </button>
                    )}
