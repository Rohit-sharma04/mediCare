...
const ForgetPasswordPage = () => {
    ...
    const onfinishHandler = async (values) => {
        ...
        try {
            ...
            const res = await axios.post("/api/v1/user/forgot-password", {email:values.email});
        ...
        <>
            <div className="h-screen pt-3 antialiased  bg-slate-200">
                <div className="max-w-lg mx-auto my-10 bg-white p-8 rounded-xl shadow shadow-slate-300">
                    ...
                    <p className="text-slate-500">Enter your email address</p>

                    <form onSubmit={handleSubmit(onfinishHandler)} className="my-10">
                        <div className="flex flex-col space-y-5">
                            <label htmlFor="email">
                                <p className="font-medium text-slate-700 pb-2">Email address</p>
                                <input id="email" type="email" className="w-full py-3 border border-slate-200 rounded-lg px-3 focus:outline-none focus:border-slate-500 hover:shadow" placeholder="Enter email address"
